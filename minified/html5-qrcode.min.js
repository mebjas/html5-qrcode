...
var Html5Qrcode=function(){function Html5Qrcode(elementId){_classCallCheck(this,Html5Qrcode);this._elementId=elementId;this._foreverScanTimeout=null;this._localMediaStream=null;this._shouldScan=!0;this._url=window.URL||window.webkitURL||window.mozURL||window.msURL;this._userMedia=navigator.getUserMedia||navigator.webkitGetUserMedia||navigator.mozGetUserMedia||navigator.msGetUserMedia}
_createClass(Html5Qrcode,[{key:"start",value:function start(cameraId,configuration,qrCodeSuccessCallback,qrCodeErrorCallback){if(!cameraId){throw "cameraId is required"}
...
var $this=this;var config=configuration?configuration:{};config.fps=config.fps?config.fps:Html5Qrcode.SCAN_DEFAULT_FPS;var isShadedBoxEnabled=config.qrbox!=undefined;var element=document.getElementById(this._elementId);var width=element.clientWidth?element.clientWidth:Html5Qrcode.DEFAULT_WIDTH;var height=element.clientHeight?element.clientHeight:Html5Qrcode.DEFAULT_HEIGHT;if(isShadedBoxEnabled){var qrboxSize=config.qrbox;if(qrboxSize<Html5Qrcode.MIN_QR_BOX_SIZE){throw "minimum size of 'config.qrbox' is ".concat(Html5Qrcode.MIN_QR_BOX_SIZE,"px.")}
if(qrboxSize>width||qrboxSize>height){throw "'config.qrbox' should not be greater than the "+"width and height of the HTML element."}}
var qrRegion=isShadedBoxEnabled?this._getShadedRegionBounds(width,height,config.qrbox):{x:0,y:0,width:width,height:height};var videoElement=this._createVideoElement(width,height);var canvasElement=this._createCanvasElement(qrRegion.width,qrRegion.height);var context=canvasElement.getContext('2d');context.canvas.width=qrRegion.width;context.canvas.height=qrRegion.height;element.style.position="relative";element.append(videoElement);element.append(canvasElement);if(isShadedBoxEnabled){this._possiblyInsertShadingElement(element,height,qrRegion)}
this._element=element;this._videoElement=videoElement;this._canvasElement=canvasElement;this._shouldScan=!0;qrcode.callback=qrCodeSuccessCallback;var foreverScan=function foreverScan(){if(!$this._shouldScan){return}
if($this._localMediaStream){context.drawImage(videoElement,qrRegion.x,qrRegion.y,qrRegion.width,qrRegion.height,0,0,qrRegion.width,qrRegion.height);try{qrcode.decode()}catch(exception){qrCodeErrorCallback("QR code parse error, error = ".concat(exception))}}
$this._foreverScanTimeout=setTimeout(foreverScan,Html5Qrcode._getTimeoutFps(config.fps))};var getUserMediaSuccessCallback=function getUserMediaSuccessCallback(stream){videoElement.srcObject=stream;videoElement.play();$this._localMediaStream=stream;foreverScan()};return new Promise(function(resolve,reject){if(navigator.mediaDevices&&navigator.mediaDevices.getUserMedia){navigator.mediaDevices.getUserMedia({audio:!1,video:{deviceId:{exact:cameraId}}}).then(function(stream){getUserMediaSuccessCallback(stream);resolve()}).catch(function(err){reject("Error getting userMedia, error = ".concat(err))})}else if(navigator.getUserMedia){var getCameraConfig={video:{optional:[{sourceId:cameraId}]}};navigator.getUserMedia(getCameraConfig,function(stream){getUserMediaSuccessCallback(stream);resolve()},function(err){reject("Error getting userMedia, error = ".concat(err))})}else{reject("Web camera streaming not supported by the browser.")}})}},{key:"stop",value:function stop(){this._shouldScan=!1;clearTimeout(this._foreverScanTimeout);var $this=this;return new Promise(function(resolve,reject){var tracksToClose=$this._localMediaStream.getVideoTracks().length;var tracksClosed=0;var removeQrRegion=function removeQrRegion(){while($this._element.getElementsByClassName(Html5Qrcode.SHADED_REGION_CLASSNAME).length){var shadedChild=$this._element.getElementsByClassName(Html5Qrcode.SHADED_REGION_CLASSNAME)[0];$this._element.removeChild(shadedChild)}};var onAllTracksClosed=function onAllTracksClosed(){$this._localMediaStream=null;$this._element.removeChild($this._videoElement);$this._element.removeChild($this._canvasElement);removeQrRegion();resolve(!0)};$this._localMediaStream.getVideoTracks().forEach(function(videoTrack){videoTrack.stop();++tracksClosed;if(tracksClosed>=tracksToClose){onAllTracksClosed()}})})}},{key:"_createCanvasElement",value:function _createCanvasElement(width,height){var canvasWidth=width;var canvasHeight=height;var canvasElement=document.createElement('canvas');canvasElement.style.width="".concat(canvasWidth,"px");canvasElement.style.height="".concat(canvasHeight,"px");canvasElement.style.display="none";canvasElement.id='qr-canvas';return canvasElement}},{key:"_createVideoElement",value:function _createVideoElement(width,height){var videoElement=document.createElement('video');videoElement.style.height="".concat(height,"px");videoElement.style.width="".concat(width,"px");videoElement.muted=!0;videoElement.playsInline=!0;return videoElement}},{key:"_getShadedRegionBounds",value:function _getShadedRegionBounds(width,height,qrboxSize){if(qrboxSize>width||qrboxSize>height){throw "'config.qrbox' should not be greater than the "+"width and height of the HTML element."}
return{x:(width-qrboxSize)/2,y:(height-qrboxSize)/2,width:qrboxSize,height:qrboxSize}}},{key:"_possiblyInsertShadingElement",value:function _possiblyInsertShadingElement(element,height,qrRegion){if(qrRegion.x==0&&qrRegion.y==0){return}
element.append(this._createShadedElement(height,qrRegion,Html5Qrcode.SHADED_LEFT));element.append(this._createShadedElement(height,qrRegion,Html5Qrcode.SHADED_RIGHT));element.append(this._createShadedElement(height,qrRegion,Html5Qrcode.SHADED_TOP));element.append(this._createShadedElement(height,qrRegion,Html5Qrcode.SHADED_BOTTOM))}},{key:"_createShadedElement",value:function _createShadedElement(height,qrRegion,shadingPosition){var elem=document.createElement('div');elem.style.position="absolute";elem.style.height="".concat(height,"px");elem.className=Html5Qrcode.SHADED_REGION_CLASSNAME;elem.id="".concat(Html5Qrcode.SHADED_REGION_CLASSNAME,"_").concat(shadingPosition);elem.style.background="#0000007a";switch(shadingPosition){case Html5Qrcode.SHADED_LEFT:elem.style.top="0px";elem.style.left="0px";elem.style.width="".concat(qrRegion.x,"px");elem.style.height="".concat(height,"px");break;case Html5Qrcode.SHADED_RIGHT:elem.style.top="0px";elem.style.right="0px";elem.style.width="".concat(qrRegion.x,"px");elem.style.height="".concat(height,"px");break;case Html5Qrcode.SHADED_TOP:elem.style.top="0px";elem.style.left="".concat(qrRegion.x,"px");elem.style.width="".concat(qrRegion.width,"px");elem.style.height="".concat(qrRegion.y,"px");break;case Html5Qrcode.SHADED_BOTTOM:elem.style.bottom="0px";elem.style.left="".concat(qrRegion.x,"px");elem.style.width="".concat(qrRegion.width,"px");elem.style.height="".concat(qrRegion.y,"px");break;default:throw "Unsupported shadingPosition"}
return elem}}],[{key:"getCameras",value:function getCameras(){var _this=this;return new Promise(function(resolve,reject){if(navigator.mediaDevices&&navigator.mediaDevices.enumerateDevices&&navigator.mediaDevices.getUserMedia){_this._log("navigator.mediaDevices used");navigator.mediaDevices.getUserMedia({audio:!1,video:!0}).then(function(ignore){navigator.mediaDevices.enumerateDevices().then(function(devices){var results=[];for(var i=0;i<devices.length;i++){var device=devices[i];if(device.kind=="videoinput"){results.push({id:device.deviceId,label:device.label})}}
_this._log("".concat(results.length," results found"));resolve(results)}).catch(function(err){reject("".concat(err.name," : ").concat(err.message))})}).catch(function(err){reject("".concat(err.name," : ").concat(err.message))})}else if(MediaStreamTrack&&MediaStreamTrack.getSources){_this._log("MediaStreamTrack.getSources used");var callback=function callback(sourceInfos){var results=[];for(var i=0;i!==sourceInfos.length;++i){var sourceInfo=sourceInfos[i];if(sourceInfo.kind==='video'){results.push({id:sourceInfo.id,label:sourceInfo.label})}}
_this._log("".concat(results.length," results found"));resolve(results)};MediaStreamTrack.getSources(callback)}else{_this._log("unable to query supported devices.");reject("unable to query supported devices.")}})}},{key:"_getTimeoutFps",value:function _getTimeoutFps(fps){return 1000/fps}},{key:"_log",value:function _log(message){if(Html5Qrcode.VERBOSE){console.log(message)}}}]);return Html5Qrcode}();_defineProperty(Html5Qrcode,"DEFAULT_HEIGHT",300);_defineProperty(Html5Qrcode,"DEFAULT_HEIGHT_OFFSET",2);_defineProperty(Html5Qrcode,"DEFAULT_WIDTH",300);_defineProperty(Html5Qrcode,"DEFAULT_WIDTH_OFFSET",2);_defineProperty(Html5Qrcode,"SCAN_DEFAULT_FPS",2);_defineProperty(Html5Qrcode,"MIN_QR_BOX_SIZE",50);_defineProperty(Html5Qrcode,"SHADED_LEFT",1);_defineProperty(Html5Qrcode,"SHADED_RIGHT",2);_defineProperty(Html5Qrcode,"SHADED_TOP",3);_defineProperty(Html5Qrcode,"SHADED_BOTTOM",4);_defineProperty(Html5Qrcode,"SHADED_REGION_CLASSNAME","qr-shaded-region");_defineProperty(Html5Qrcode,"VERBOSE",!1)